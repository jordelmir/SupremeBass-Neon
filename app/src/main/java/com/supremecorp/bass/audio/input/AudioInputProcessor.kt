 ...
 *
 * Architecture: PULL-BASED (no internal thread)
 *   - Caller calls read() when they want data
 *   - Single producer (AudioRecord) → caller controls consumption
 *   - No double-read, no concurrency issues
 *
 * Requirements:
...
class AudioInputProcessor(private val context: Context) {
    ...
    // Callback for when data is available (optional — for push mode if needed)
     ...
     *
     * This is a PULL operation — caller controls when to read.
     * No internal thread, no double-read, no concurrency issues.
     */
